        ...
        for (int iter=1; iter<=Max_iteration; iter++){
            ...
            for (int i=0; i<SearchAgents_no; i++){  //Calculate all the objective values first
                if (dim <= 20){
                    ...
                    for (int j=0; j<dim; j++){
                        x_OBL[j] = (1.0 - (double) iter *(1.0 - 0.1)/ (double) Max_iteration) * (lb[j]+ ub[j] - X[i][j]);
                    }
...
//                }
                    ...
                    if (fobj.func(x_OBL) < fobj.func(X[i])){
                        for (int j=0; j<dim; j++){
                            X[i][j] = x_OBL[j];
                        }
                    }
//
//                if (fobj.func(x_MCS) < fobj.func(X[i])){
//                    for (int j=0; j<dim; j++){
//                        X[i][j] = x_MCS[j];
//                    }
//                }
//
//                if (fobj.func(x_MCS_OBL) < fobj.func(X[i])){
//                    for (int j=0; j<dim; j++){
//                        X[i][j] = x_MCS_OBL[j];
//                    }
//                }
