...
public class HSCA implements Algorithm {
    double r1_gwo;
    double r2_gwo;
    int N, N_GWO, N_SCA;
    int dim;
    ...
    double[][] solution(){
        ...
        while(iter <= maxiter) {
            ...
            X_GWO = simplebounds(X_GWO, N_GWO);
            //Hop nhat GWO va SCA
            for (int i=0; i<N_GWO; i++){
                for (int j=0; j<dim; j++){
                    X[i][j]=X_GWO[i][j];
            ...
            if (iter > 1){
                if (ff.func(arrRandomBestVal[iter-2]) < ff.func(arrRandomBestVal[iter-1])){
                    for(int i = 0; i < dim; i++) {
                        arrRandomBestVal[iter-1][i] = arrRandomBestVal[iter-2][i];
                    }
                }
